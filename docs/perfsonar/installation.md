...
latest available release. In case you have already an instance running and wish to re-install/update it then please
...
* Upgrades: We recommend *reinstalling* using an EL9 (RHEL,Rocky,Alma) OS for all sites already running a registered
  ...
  OS and to benefit from a 5.x kernel.
...
The following options are then recommended to install perfSONAR for OSG/WLCG:
 | Installation method | Link | | --- | --- | | Toolkit bundle installation | [Toolkit Installation Quick
...
[guide](http://docs.perfsonar.net/manage_update.html).
 Using automated configuration tools (such as Chef, Puppet, etc) for managing perfSONAR are not officially supported,
but there are some community driven projects that could be helpful, such as [HEP-
