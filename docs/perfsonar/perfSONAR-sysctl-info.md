...
performance test and measurement hosts and are sufficient for most use cases. However, you can manually verify or adjust
the settings in `/etc/sysctl.conf` for specific needs.

## TCP buffer sizing

These settings increase the maximum TCP buffer sizes to support high throughput, especially on high-speed (e.g., 10 Gbps
and faster) networks over long distances.
...
If you are running perfSONAR on a host with multiple network interfaces on the same subnet, specific `sysctl` settings
are needed to prevent Address Resolution Protocol (ARP) conflicts.
...
For installations using the perfSONAR Toolkit, the included `perfsonar-toolkit-sysctl` package handles most tuning
automatically. If you change an interface speed, you can re-run the tuning script to apply appropriate settings:
